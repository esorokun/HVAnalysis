...
def main(args):
    ...
    del df_l['sumcurr']
    del df_l['sumvolt']
    del df_l['stable_original']
    del df_l['resistance']
    del df_l['avgvolt']
    df_l = df_l.bfill(axis=0)
    print(df_l)
    pyod_model = CBLOF(contamination=0.15)
    pyod_sktime_annotator = PyODAnnotator(pyod_model)
    pyod_sktime_annotator.fit(df_l)
    df['result'] = pyod_sktime_annotator.predict(df_l)
    df['datetime'] = df.index
    df_red = df.loc[df['result'] == 1]
    df_blue = df.loc[df['result'] == 0]
    fig, axes = plt.subplots(1, 2)
    fig = sns.scatterplot(x='datetime', y='avgcurr', data=df_red, s=2, color='r', ax=axes[0])
    axes = sns.scatterplot(x='datetime', y='avgcurr', s=2, data=df_blue, color='b', ax=axes[1])
